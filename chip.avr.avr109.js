var intelHex = require('intel-hex'), Stream = require('stream').Stream,
    util = require('util');
...
out.Flasher = function(serialport, options) {
  ...
  this.c =
      function(value, fn, expectedResponseLength) {
    that.cmds.push({
      value: value,
...
out.Flasher.prototype = {
    ...
    process.nextTick(function() {
      ...
      if (cmd) {
        ...
        var onData = function(d) {
          ...
          if (cmd.expectedResponseLength ===
                  undefined ||  // optional expected length not passed in
    ...
    this.c('S',
           function(d) {
             if (d.toString() !== that.signature) {
               fn(new Error(
                   'Invalid device signature; expecting: ' + that.signature +
                   ' received: ' + d.toString()));
             }
           })
        .c('V')
        ...
        .c('Q')
        .c([d('A'), 0x03, 0xfc])
        .c([d('g'), 0x00, 0x01, d(d(that.options.eeprom ? 'E' : 'F'))])
        .c([d('A'), 0x03, 0xff])
        .c([d('g'), 0x00, 0x01, d(d(that.options.eeprom ? 'E' : 'F'))])
        .c([d('A'), 0x03, 0xff])
        .c([d('g'), 0x00, 0x01, d(d(that.options.eeprom ? 'E' : 'F'))])
  ...
  program: function(fullString, fn) {
    ...
    this.c([d('A'), 0x00, 0x00], function() {
      ...
      for (var i = 0; i < bytes.length; i += that.flashChunkSize) {
        var chunk = Array.prototype.slice.call(
            converter.data.slice(i, i + that.flashChunkSize));
        that.chunksSent.push(chunk);
        that.c([
          d('B'), (chunk.length >> 8) & 0xFF, chunk.length & 0xFF,
          d(that.options.eeprom ? 'E' : 'F')
        ].concat(chunk));
      }
    });
  ...
  verify: function(fn) {
    ...
    this.c([d('A'), 0x00, 0x00], function() {
      var index = 0, compare = function(deviceData) {
        var error = null;
        index++;
        ...
        // iterate through the bytes sent to compare with the latest bytes
        // received
        localChunk.forEach(function(val, idx) {
          if (val !== deviceData.readUInt8(idx)) {
            error =
                new Error('Firmware on the device does not match local data');
          }
        });
        ...
        process.nextTick(function() {
          ...
          that.options.debug &&
              console.log(that.totalBytes - index * that.flashChunkSize);
          if (that.totalBytes - index * that.flashChunkSize <
              that.flashChunkSize) {
            readSize = that.totalBytes - index * that.flashChunkSize;
          }
          that.c(
              [
                d('g'), (readSize >> 8) & 0xFF, readSize & 0xFF,
                d(d(that.options.eeprom ? 'E' : 'F'))
              ],
              compare, readSize);
          that.run();
        });
      ...
      that.c(
          [
            d('g'), (that.flashChunkSize >> 8) & 0xFF,
            that.flashChunkSize & 0xFF, d(d(that.options.eeprom ? 'E' : 'F'))
          ],
          compare, that.flashChunkSize);
      that.run();
    });
    that.run();
  },
